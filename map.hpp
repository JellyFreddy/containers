...
namespace ft {
	template<
	    ...
	    private:
			...
			tree_type _treeElem;
			tree_ptr _tree;
			node_type _lastElemNode;
			node_ptr _lastElem;
			key_compare _comparator;
			allocator_type _allocator;

			void setTreeAddress(const tree_ptr newTree) { _tree = newTree; }
			void setLastElemAddress(const node_ptr newLastElem) { _lastElem = newLastElem; }
			void setTree(const tree_type & newTree) { _treeElem = newTree; }
			void setLast(const node_type & newLastElem) { _lastElemNode = newLastElem; }
		public:
	    	map() {
	    		_treeElem = AVLTree<Key, T, Compare, Allocator>();
	    		_tree = &(_treeElem);
	    		_lastElemNode = TreeNode<T, Key, Allocator>();
	    		_lastElem = &(_lastElemNode);
	    	...
	    	map& operator=( const map& other ) {
	    		...
	    		clear();
	    		_lastElemNode = TreeNode<T, Key, Allocator>(*(other._lastElem));
	    		_treeElem = AVLTree<Key, T, Compare, Allocator>();
	    		for (const_iterator it = other.begin(); it != other.end(); ++it)
	    			_tree->insert((*it).first, (*it).second);
	    		return (*this);
	    	...
	    	iterator begin() {
	    		_tree->getFirstElem();
	    		return map_iterator<Key, T, Compare, tree_type, Allocator>(_tree->getFirstElem(), _lastElem, _tree);
	    	}
	    	...
	    	iterator end() {
	    		return map_iterator<Key, T, Compare, tree_type, Allocator>(_lastElem, _lastElem, _tree);
	    	}
	    	...
	    	void swap( map& other ) {
	    		node_ptr tempRoot = _tree->getRoot();
				_tree->setRoot(other._tree->getRoot());
				other._tree->setRoot(tempRoot);
